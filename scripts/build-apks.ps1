# Marka-parametrik APK build (Windows). Iki rol (garson + yonetici) icin release APK uretir.
...
# Ilk calismada JDK17 + Android SDK indirir (~1GB). Sonraki calismalarda hizli.
param(
  [ValidateSet('alkyone','sakura')]
  [string]$Brand = 'alkyone'
)
...
function Log($m) { Write-Host "[apk:$Brand] $m" -ForegroundColor Cyan }
...
if (-not (Test-Path $javaBin)) {
  ...
  if ($inner) { Get-ChildItem $inner.FullName | Move-Item -Destination $JDK_HOME -Force; Remove-Item $inner.FullName -Recurse -Force }
  Remove-Item $jdkZip
}
...
if (-not (Test-Path $sdkmgr)) {
  ...
  $unzipDir = "$env:TEMP\pos-cmdtools"
  ...
  Remove-Item $tzip; Remove-Item $unzipDir -Recurse -Force
}
...
if (-not (Test-Path $plat) -or -not (Test-Path $bt)) {
  Log 'SDK lisanslari + paketleri indiriliyor (~600MB)...'
  $ys = ('y' + "`n") * 30
...
# ===== Gradle build (marka + iki rol) =====
Log "Gradle build: -PposBrand=$Brand (garson + yonetici release)"
Push-Location (Join-Path $ROOT 'android')
"sdk.dir=$($SDK_ROOT -replace '\\','\\')" | Set-Content -Path 'local.properties' -Encoding ascii
& cmd /c "gradlew.bat --no-daemon -PposBrand=$Brand :app:assembleGarsonRelease :app:assembleYoneticiRelease"
$exit = $LASTEXITCODE
Pop-Location
if ($exit -ne 0) { throw "Gradle build basarisiz: $exit" }

# ===== Ciktilari marka klasorune kopyala =====
$outDir = Join-Path $ROOT "dist\apk\$Brand"
